...
// AzureOpenAI, or maybe even AWS Bedrock.
//
// So the `modelConfigInfo.Provider` isn't really necessary. Here we just map the
// `modelConfigInfo.Provider`'s `ServerSideConfig` to an API Provider which will serve
// the actual request.
