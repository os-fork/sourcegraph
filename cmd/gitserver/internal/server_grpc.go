...
func (gs *grpcServer) OctopusMergeBase(ctx context.Context, req *proto.OctopusMergeBaseRequest) (*proto.OctopusMergeBaseResponse, error) {
	revspecs := byteSlicesToStrings(req.GetRevspecs())
	accesslog.Record(
		ctx,
		req.GetRepoName(),
		log.Strings("revspecs", revspecs),
	)
	...
	sha, err := backend.OctopusMergeBase(ctx, revspecs...)
	if err != nil {
		var e *gitdomain.RevisionNotFoundError
		if errors.As(err, &e) {
			s, err := status.New(codes.NotFound, "revision not found").WithDetails(&proto.RevisionNotFoundPayload{
	...
	return &proto.OctopusMergeBaseResponse{
		MergeBaseCommitSha: string(sha),
	}, nil
}
