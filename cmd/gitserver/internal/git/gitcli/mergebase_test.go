...
func TestGitCLIBackend_OctopusMergeBase(t *testing.T) {
	ctx := context.Background()
	...
	t.Run("resolves", func(t *testing.T) {
		...
		base, err := backend.OctopusMergeBase(ctx, "master", "b2", "b3")
		require.NoError(t, err)
	...
	t.Run("not found revspec", func(t *testing.T) {
		...
		_, err := backend.OctopusMergeBase(ctx, "master", "notfound")
		require.Error(t, err)
		require.True(t, errors.HasType[*gitdomain.RevisionNotFoundError](err))

		// First revspec not found
		_, err = backend.OctopusMergeBase(ctx, "notfound", "master")
		require.Error(t, err)
		require.True(t, errors.HasType[*gitdomain.RevisionNotFoundError](err))
	})
	t.Run("less than two revspecs", func(t *testing.T) {
		...
		_, err := backend.OctopusMergeBase(ctx, "master")
		require.Error(t, err)
