...
func ResetMock() error {
	...
	// Looks like all is well. So we rebuild the model configuration.
	// We do not take static config or Cody Gateway config into account because
	// it would just lead to indeterminism in tests.
	...
	var (
		siteModelConfig *types.SiteModelConfiguration
		err             error
	)
